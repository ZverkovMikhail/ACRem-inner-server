...
<div class="container" id="container">
            <p>Тут будет пульт и кномка меню, но пока так!))</p>
            <a class="button" href="/settings/wifi">настройки WiFi</a>
            <a class="button" href="/settings/mqtt">настройки MQTT</a>
</div>
...
<script>
    const ip = '%IP_ADDR%';
    let socket = new WebSocket(`ws://${ip}/settings`);
    ...
    socket.onclose = function (event) {
        if (event.wasClean) {
            ...
            socket = new WebSocket(`ws://${ip}/settings`);
        }
    };
...
<div class="container" id="container">
    ...
    <div style="display:flex; flex-direction: column; align-items: center;" action='action' method='post'>
        <h2>Настройки</h2>
        <div class="settings-wrap">
            <label for="user_key_id">User key:</label>
            <input class="text-input" type='text' id='user_key_id' value=''>
            <label for="dev_id">Device id:</label>
            <input class="text-input" type='text' id='dev_id' value=''>
